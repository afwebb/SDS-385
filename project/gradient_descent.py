...
def run_descent(e, step_size):
    ...
    while not test_converge(l, e):
        ...
        if i%20==0:
            step_size=0.8*step_size #decrease step size over time
            print i
...
def predict_acc(b):
    ...
    return score_sig, score_bkg, float(n_correct)/float(len(y)), significance
...
plt.savefig('plots/result_gradient.png', format = 'png')

#b_tot = b_tot/np.linalg.norm(b_tot, ord=1)
score_sig, score_bkg, acc, significance = predict_acc(b)
print "Accuracey of prediction: "+str(acc)
print "Significance of signal: "+str(significance)

plt.figure(1)
plt.hist([score_sig,score_bkg], label="Signal")
plt.savefig('plots/gradient_dis.png', format= 'png')
